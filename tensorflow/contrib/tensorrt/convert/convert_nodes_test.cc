...
TEST_F(OpConverterTest, ConvertShape) {
  {
    // Input list is empty, should fail.
    NodeDef node_def = MakeNodeDef("my_shape", "Shape", {});
    RunConversion(
        node_def, error::INVALID_ARGUMENT,
        "Shape op expects 1 input, at my_shape");
  }

  // Get the NodeDef for Shape.
  Scope s = Scope::NewRootScope();
  auto input = ops::Placeholder(s.WithOpName("input"), DT_FLOAT);
  ...
  //   RunConversion(node_def);
  //   TRT_TensorOrWeights output;
  //   TF_EXPECT_OK(GetTensorOrWeights("my_shape", &output));
  //   EXPECT_TRUE(output.is_weights());
  //   EXPECT_TRUE(TrtDimsEqualsArray({4}, output.GetTrtDims()))
  //       << output.DebugString();

  //   std::vector<float> output_data(6);
  //   BuildAndRun("input", {1, 2, 3, 4, 5, 6}, "my_shape", &output_data);
  //   EXPECT_THAT(output_data, ElementsAre(1, 1, 2, 3));
  // }
}
